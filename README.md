...
same set on every push and PR, blocks the build if any of them fails,
and on a `vX.Y.Z` tag publishes a GitHub Release containing:

- the Linux x64 binary
- `LICENSE`
- `THIRD_PARTY_LICENSES.txt` — verbatim license + copyright text for every
  third-party crate the binary links, generated by `cargo-about`
- `kpcli-rust.cdx.json` — CycloneDX 1.5 software bill of materials
  (every dep with version, hash, and license) generated by
  `cargo-cyclonedx`
- `.tar.gz`, `.deb`, and `.rpm` packages
