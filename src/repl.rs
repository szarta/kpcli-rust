...
impl Shell {
    ...
    fn cmd_add(&mut self, args: &[&str]) -> Result<()> {
        ...
        // Collect field values; password via rpassword so it never echoes
        // and never enters rustyline's history.
        let username = prompt_line("Username (blank to skip): ")?;
        let password = Zeroizing::new(rpassword::prompt_password(
            "Password (blank to skip, hidden): ",
        )?);
        let url = prompt_line("URL (blank to skip): ")?;
        let notes = prompt_line("Notes (blank to skip): ")?;
